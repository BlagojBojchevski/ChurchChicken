...
open class CustomEditTextFirstName : AppCompatEditText, View.OnTouchListener,
    View.OnFocusChangeListener,
    CustomTextWatcherNew.TextWatcherListener {
    ...
    override fun onTextChanged(view: EditText, text: String) {
       ...
       //     setClearIconVisible(isNotEmpty(text))
            ...
            view.filters = arrayOf(spaceFilter, InputFilter.LengthFilter(30))
        }


    }

    override fun onafterTextChanged(view: EditText, text: String) {
        if (!text.equals(text.toUpperCase())) {
            if (text.length == 1) {
                var txt = text
                txt = text.toUpperCase();
                view.setText(txt);
                view.setSelection(txt.length); //fix reverse texting
            }
        }
    }

    private var spaceFilter: InputFilter = InputFilter { source, start, end, text, _, _ ->
    ...
    private fun init() {
        ...
        addTextChangedListener(CustomTextWatcherNew(this, this,this))
        initIcon()
        setClearIconVisible(false)

        super.setFilters(arrayOf(spaceFilter, InputFilter.LengthFilter(30)))
    }
