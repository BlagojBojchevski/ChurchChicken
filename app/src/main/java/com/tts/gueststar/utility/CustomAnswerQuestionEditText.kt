...
open class CustomAnswerQuestionEditText : AppCompatEditText, View.OnTouchListener,
    ...
    override fun onTextChanged(view: EditText, text: String) {
        val maxLength = AppConstants.maxEmailCharacters
        val filterArea = arrayOfNulls<InputFilter>(1)
        filterArea[0] = InputFilter.LengthFilter(maxLength)
        view.filters = arrayOf(filterArea[0], spaceFilter)
        if (isFocused) {
            setClearIconVisible(isNotEmpty(text))
            if (Engine().filterSecurityAnswer(view)) {
