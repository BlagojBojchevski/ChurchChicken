...
import com.tts.gueststar.interfaces.OnSubmitReceiptLocationPicked
import com.tts.gueststar.utility.Engine
import kotlinx.android.synthetic.main.inflater_favorite_locations.view.*

class SubmitReceiptLocationsAdapter(
    val context: Context,
    private val locations: List<LocationsResponse.Location>,
    private val listener: OnSubmitReceiptLocationPicked
) : RecyclerView.Adapter<SubmitReceiptLocationsAdapter.LocationViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, p1: Int): LocationViewHolder {
        return LocationViewHolder(
            LayoutInflater.from(context).inflate(
                R.layout.inflater_submit_receipt_locations,
                parent,
                false
            )
        )
    ...
    override fun onBindViewHolder(holder: LocationViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            listener.onFavLocationPicked(locations[position])
        }
    }
